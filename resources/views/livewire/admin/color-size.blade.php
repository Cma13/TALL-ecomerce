            ...
            <div class="grid grid-cols-6 gap-6">
                @foreach ($colors as $color)
                    <label>
                        ...
                        <span class="ml-2 text-gray-700">{{ __(ucfirst($color->name)) }}</span>
                    </label>
                @endforeach
            </div>
            ...
            <table>
                ...
                <tbody>
                    @foreach ($sizeColors as $sizeColor)
                        <tr wire:key="sizeColor-{{ $sizeColor->pivot->id }}">
                            <td class="px-4 py-2">
                                {{ __(ucfirst($colors->find($sizeColor->pivot->color_id)->name)) }}
                            </td>
                            <td class="px-4 py-2">
                                {{ $sizeColor->pivot->quantity }} unidades
                            </td>
                            <td class="flex px-4 py-2">
                                <x-jet-secondary-button class="ml-auto mr-2"
                                    wire:click="edit({{ $sizeColor->pivot->id }})" wire:loading.attr="disabled"
                                    wire:target="edit({{ $sizeColor->pivot->id }})">
                                    Actualizar
                                </x-jet-secondary-button>
                                <x-jet-danger-button
                                    wire:click="$emit('deleteColorSize', {{ $sizeColor->pivot->id }})">
                                    Eliminar
                                </x-jet-danger-button>
                            </td>
                        </tr>
