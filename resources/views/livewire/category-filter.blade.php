        ...
        <aside>
            ...
            <ul class="divide-y divide-gray-200">
                @foreach ($category->subcategories as $subcategory)
                    <li class="py-2 text-sm">
                        <a class="cursor-pointer hover:text-orange-500 capitalize {{ $subcategoryFilter == $subcategory->name ? 'text-orange-500 font-semibold' : '' }}"
                            wire:click="$set('subcategoryFilter', '{{ $subcategory->name }}')">{{ $subcategory->name }}</a>
                    </li>
                @endforeach
            </ul>
            ...
            <ul class="divide-y divide-gray-200">
                @foreach ($category->brands as $brand)
                    <li class="py-2 text-sm">
                        <a class="cursor-pointer hover:text-orange-500 capitalize {{ $brandFilter == $brand->name ? 'text-orange-500 font-semibold' : '' }}"
                            wire:click="$set('brandFilter', '{{ $brand->name }}')">{{ $brand->name }}</a>
                    </li>
                @endforeach
            </ul>
                            ...
                            <article class="flex ">
                                ...
                                <div class="flex-1 py-4 px-6 flex flex-col">
                                    ...
                                    <div class="mt-auto mb-6">
                                        <x-jet-danger-button><a href="{{ route('products.show', $product) }}">Más información</a></x-jet-danger-but>
                                    </div>
                                </div>
                            </article>
                        </li>
